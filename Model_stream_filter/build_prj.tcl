...
if {$opt(reset)} {
  open_project -reset filtering_network_prj
} else {
  open_project filtering_network_prj
}
set_top filtering_network
add_files firmware/filtering_network.cpp -cflags "-std=c++0x"
add_files -tb filtering_network_test.cpp -cflags "-std=c++0x"
...
if {$opt(cosim)} {
  ...
  add_files -tb filtering_network_test.cpp -cflags "-std=c++0x -DRTL_SIM"
  set time_start [clock clicks -milliseconds]
  cosim_design -trace_level all
  set time_end [clock clicks -milliseconds]
  puts "INFO:"
  puts [read [open filtering_network_prj/solution1/sim/report/filtering_network_cosim.rpt r]]
...
if {$opt(vsynth)} {
  ...
  if {[file exist filtering_network_prj/solution1/syn/vhdl]} {
    set time_start [clock clicks -milliseconds]
