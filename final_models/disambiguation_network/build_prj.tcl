...
if {$opt(reset)} {
  open_project -reset disambiguation_network_prj
} else {
  open_project disambiguation_network_prj
}
set_top disambiguation_network
add_files firmware/disambiguation_network.cpp -cflags "-std=c++0x"
add_files -tb disambiguation_network_test.cpp -cflags "-std=c++0x"
...
if {$opt(cosim)} {
  ...
  add_files -tb disambiguation_network_test.cpp -cflags "-std=c++0x -DRTL_SIM"
  set time_start [clock clicks -milliseconds]
  cosim_design -trace_level all
  set time_end [clock clicks -milliseconds]
  puts "INFO:"
  puts [read [open disambiguation_network_prj/solution1/sim/report/disambiguation_network_cosim.rpt r]]
...
if {$opt(vsynth)} {
  ...
  if {[file exist disambiguation_network_prj/solution1/syn/vhdl]} {
    set time_start [clock clicks -milliseconds]
