...
int main(int argc, char **argv)
{
  ...
  if (fin.is_open() && fpr.is_open()) {
    while ( std::getline(fin,iline) && std::getline (fpr,pline) ) {
      ...
      std::fill_n(layer11_out, 12, 0.);

      //hls-fpga-machine-learning insert top-level-function
      unsigned short size_in1,size_out1;
      disambiguation_network(input,layer11_out,size_in1,size_out1);

      //hls-fpga-machine-learning insert tb-output
      for(int i = 0; i < N_OUTPUTS_9*N_FILT_9; i++) {
        fout << layer11_out[i] << " ";
      }
      fout << std::endl;
    ...
    input_t input[N_INPUT_1_1];
    std::fill_n(input, N_INPUT_1_1, 0.);
    result_t layer11_out[N_OUTPUTS_9*N_FILT_9]{};
      std::fill_n(layer11_out, 12, 0.);
